...
#' names(ests)
#' ests$beta
#' ExtractEsts(fitkm, par = "beta")

ExtractEsts <- function(fit, par = NULL, q = c(0.025, 0.25, 0.5, 0.75, 0.975), sel = NULL, digits = 5) {
  if(length(par) > 1){
...
#' ExtractSamps(fitkm, par = "beta")
ExtractSamps <- function(fit, par = NULL, sel = NULL) {
  if (inherits(fit, "bkmrfit")) {
    if (is.null(sel)) {
      sel <- with(fit, seq(burnin + 1, iter))
    }
